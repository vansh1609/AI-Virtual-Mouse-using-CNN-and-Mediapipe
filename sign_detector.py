...
def perform_mouse_action(letter):
    move_distance = 50  # Pixels to move the cursor
    if letter == 'L':
        pyautogui.click(button='left')
    elif letter == 'O':
        pyautogui.click(button='right')
    elif letter == 'W':
        pyautogui.moveRel(0, -move_distance)  # Move cursor up
    elif letter == 'V':
        pyautogui.moveRel(0, move_distance)   # Move cursor down
    elif letter == 'C':
        pyautogui.moveRel(-move_distance, 0)  # Move cursor left
    elif letter == 'G':
        pyautogui.moveRel(move_distance, 0)   # Move cursor right
...
while cap.isOpened():
    ...
    cv2.putText(text_window, 'Recognized Gesture:', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2)
    cv2.putText(text_window, recognized_text, (10, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
    
    # Show the recognized text window
    cv2.imshow('Recognized Gesture', text_window)
    perform_mouse_action(asl_letter)
